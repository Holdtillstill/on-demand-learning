...
grep -q "not acceptable promotion evidence" "$TRIAGE" || fail "triage-notes.md should reject latest-tag promotion"
grep -q "Deleting a secret in a later Dockerfile layer" "$TRIAGE" || fail "triage-notes.md should reject deleted-layer secret confidence"
grep -q "runAsNonRoot" "$TRIAGE" || fail "triage-notes.md should require image-level non-root evidence"
grep -q "SBOM and vulnerability scans after promotion" "$TRIAGE" || fail "triage-notes.md should reject post-promotion scans"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|False Leads|latest|deleted secret|runAsNonRoot|post-promotion|rollback digest"

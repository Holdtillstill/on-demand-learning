...
grep -q "Waiting for root cause before assigning roles" "$TRIAGE" || fail "triage-notes.md should reject delayed role assignment"
grep -q "Silence until the fix is confirmed" "$TRIAGE" || fail "triage-notes.md should reject delayed communications"
grep -q "Immediate rollback without a decision criterion" "$TRIAGE" || fail "triage-notes.md should reject rollback without criteria"
grep -q "Timeline entries written after the incident" "$TRIAGE" || fail "triage-notes.md should reject late timeline writing"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|False Leads|root cause|stakeholder update|decision criterion|timeline later"

...
grep -q "Target.ResponseCodeMismatch" "$TRIAGE" || fail "triage-notes.md should include ALB target-health evidence"
grep -q "targetPort: web" "$TRIAGE" || fail "triage-notes.md should include Service targetPort evidence"
grep -q "console-only" "$TRIAGE" || fail "triage-notes.md should reject console-only fixes"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|triage notes|False Leads|false leads|console-only|security groups"

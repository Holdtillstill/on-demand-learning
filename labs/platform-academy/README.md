...
- [`inspect-linux-failure-evidence`](inspect-linux-failure-evidence/README.md): triage notes, false leads, CrashLoopBackOff, exit code 126, permission denied, runtime UID/GID, rejected memory/root/live-chmod workarounds, and image-permission remediation evidence.
...
- [`design-production-eks-review`](design-production-eks-review/README.md): triage notes, false leads, endpoint posture, missing PDB, zonal storage, cost labels, upgrade/add-on risk, launch blockers, owners, and no-AWS review evidence.
- [`review-terraform-eks-plan`](review-terraform-eks-plan/README.md): triage notes, false leads, EKS node group replacement, subnet and capacity regression, public ingress, broad IAM scope, rollback, and approval decision.

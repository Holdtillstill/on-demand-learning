...
grep -q "A green build is not release approval" "$TRIAGE" || fail "triage-notes.md should reject green-build-only approval"
grep -q "A commit SHA tag is not the same as immutable digest promotion" "$TRIAGE" || fail "triage-notes.md should reject SHA-tag-only promotion"
grep -q "Running scans after production deploy" "$TRIAGE" || fail "triage-notes.md should reject post-deploy scans as a gate"
grep -q "A rollback button is not enough" "$TRIAGE" || fail "triage-notes.md should require rollback digest evidence"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|False Leads|green build|SHA tag|post-deploy scans|rollback digest"

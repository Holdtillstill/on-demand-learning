...
grep -q "system:serviceaccount:default:checkout" "$TRIAGE" || fail "triage-notes.md should include broken trust evidence"
grep -q "s3:PutObject" "$TRIAGE" || fail "triage-notes.md should include permission evidence"
grep -q "Wildcard trust" "$TRIAGE" || fail "triage-notes.md should reject wildcard trust"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|triage notes|False Leads|false leads|token rotation|wildcard"

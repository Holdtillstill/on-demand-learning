...
grep -q "FailedCreatePodSandBox" "$TRIAGE" || fail "triage-notes.md should include sandbox failure evidence"
grep -q "subnet-bbb222" "$TRIAGE" || fail "triage-notes.md should include constrained subnet evidence"
grep -q "Blind node scaling" "$TRIAGE" || fail "triage-notes.md should reject blind node scaling"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|triage notes|False Leads|false leads|Blind node scaling|application Pods"

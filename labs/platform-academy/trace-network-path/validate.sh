...
grep -q "Hop-By-Hop Evidence" "$HOP_TRACE" || fail "hop-trace.md should include hop-by-hop evidence"
grep -q "False Leads Ruled Out" "$HOP_TRACE" || fail "hop-trace.md should include false leads"
grep -q "targetPort: http" "$HOP_TRACE" || fail "hop-trace.md should include the source fix"
...
grep -q "Hop trace reviewed" "$TEMPLATE" || fail "evidence-template.md should prompt for hop trace evidence"
grep -q "## False Leads Ruled Out" "$TEMPLATE" || fail "evidence-template.md should prompt for false leads"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "hop trace and false leads" "hop-trace\\.md|Hop Trace|hop trace|False Leads|false leads|DNS is not|console-only"

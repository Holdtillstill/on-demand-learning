...
grep -q "Rollback is not the first command" "$TRIAGE" || fail "triage-notes.md should reject rollback-first response"
grep -q "Revision 43 correlation is not proof" "$TRIAGE" || fail "triage-notes.md should reject correlation-only proof"
grep -q "Follow-up ownership is not optional" "$TRIAGE" || fail "triage-notes.md should require follow-up ownership"
grep -q "## Triage Notes And False Leads" "$EVIDENCE_TEMPLATE" || fail "evidence-template.md should prompt for triage false leads"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|False Leads|rollback first|Revision 43 correlation|user impact|ownerless follow-up"

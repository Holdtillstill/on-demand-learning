...
grep -q "Force-sync is not the first action" "$TRIAGE" || fail "triage-notes.md should reject force-sync first"
grep -q "/spec/replicas" "$TRIAGE" || fail "triage-notes.md should include exact replica field evidence"
grep -q "Ignoring the whole Deployment is unsafe" "$TRIAGE" || fail "triage-notes.md should reject broad ignore"
...
if [[ -n "$evidence_file" ]]; then
  ...
  require_evidence_match "$evidence_file" "triage notes and false leads" "triage-notes\\.md|triage notes|False Leads|false leads|force-sync|whole Deployment"

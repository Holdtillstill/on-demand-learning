...
"$LAB_ROOT/run-lab.sh" setup design-production-eks-review --run-analyzer --evidence "$tmpdir/production-eks-review-analyzer-evidence.md" >"$tmpdir/runner-production-eks-review-analyzer-setup.txt"
grep -q "Production EKS review analysis passed" "$tmpdir/runner-production-eks-review-analyzer-setup.txt" || fail "Production EKS review setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup write-slo-backed-runbook --run-analyzer --evidence "$tmpdir/slo-runbook-analyzer-evidence.md" >"$tmpdir/runner-slo-runbook-analyzer-setup.txt"
grep -q "SLO runbook analysis passed" "$tmpdir/runner-slo-runbook-analyzer-setup.txt" || fail "SLO runbook setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup design-safe-release-pipeline --run-analyzer --evidence "$tmpdir/release-pipeline-analyzer-evidence.md" >"$tmpdir/runner-release-pipeline-analyzer-setup.txt"
grep -q "Safe release pipeline analysis passed" "$tmpdir/runner-release-pipeline-analyzer-setup.txt" || fail "Safe release pipeline setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup create-platform-golden-path --run-analyzer --evidence "$tmpdir/golden-path-analyzer-evidence.md" >"$tmpdir/runner-golden-path-analyzer-setup.txt"
grep -q "Golden path readiness analysis passed" "$tmpdir/runner-golden-path-analyzer-setup.txt" || fail "Golden path setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup review-docker-image-supply-chain --run-analyzer --evidence "$tmpdir/docker-supply-chain-analyzer-evidence.md" >"$tmpdir/runner-docker-supply-chain-analyzer-setup.txt"
grep -q "Docker supply-chain analysis passed" "$tmpdir/runner-docker-supply-chain-analyzer-setup.txt" || fail "Docker supply-chain setup --run-analyzer should execute the analyzer"
...
grep -q "Career artifact pack analysis passed" "$tmpdir/runner-career-proof-analyzer-setup.txt" || fail "Career artifact setup --run-analyzer should execute the analyzer"
...
grep -q "CrashLoop/ImagePull analysis passed" "$tmpdir/runner-crashloop-imagepull-analyzer-setup.txt" || fail "CrashLoop/ImagePull setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup review-yaml-before-apply --run-analyzer --evidence "$tmpdir/yaml-review-analyzer-evidence.md" >"$tmpdir/runner-yaml-review-analyzer-setup.txt"
grep -q "YAML manifest risk analysis passed" "$tmpdir/runner-yaml-review-analyzer-setup.txt" || fail "YAML review setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup trace-argocd-drift --run-analyzer --evidence "$tmpdir/argocd-drift-analyzer-evidence.md" >"$tmpdir/runner-argocd-drift-analyzer-setup.txt"
grep -q "ArgoCD drift analysis passed" "$tmpdir/runner-argocd-drift-analyzer-setup.txt" || fail "ArgoCD drift setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup inspect-linux-failure-evidence --run-analyzer --evidence "$tmpdir/linux-failure-analyzer-evidence.md" >"$tmpdir/runner-linux-failure-analyzer-setup.txt"
grep -q "Linux failure evidence analysis passed" "$tmpdir/runner-linux-failure-analyzer-setup.txt" || fail "Linux failure setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup trace-network-path --run-analyzer --evidence "$tmpdir/network-path-analyzer-evidence.md" >"$tmpdir/runner-network-path-analyzer-setup.txt"
grep -q "Network path analysis passed" "$tmpdir/runner-network-path-analyzer-setup.txt" || fail "Network path setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup debug-aws-alb-health-path --run-analyzer --evidence "$tmpdir/alb-health-path-analyzer-evidence.md" >"$tmpdir/runner-alb-health-path-analyzer-setup.txt"
grep -q "ALB health path analysis passed" "$tmpdir/runner-alb-health-path-analyzer-setup.txt" || fail "ALB health path setup --run-analyzer should execute the analyzer"
"$LAB_ROOT/run-lab.sh" setup audit-tenant-boundaries --run-analyzer --evidence "$tmpdir/tenant-boundaries-analyzer-evidence.md" >"$tmpdir/runner-tenant-boundaries-analyzer-setup.txt"
grep -q "Tenant boundary analysis passed" "$tmpdir/runner-tenant-boundaries-analyzer-setup.txt" || fail "Tenant boundaries setup --run-analyzer should execute the analyzer"

...
The local plan analyzer summarizes the same block decision without Terraform or AWS access:
...
A good handoff says: do not apply this plan because it replaces `module.eks.aws_eks_node_group.apps`, reduces subnet coverage from two subnets to one, cuts desired capacity from 6 to 3, opens `443` to `0.0.0.0/0`, and keeps `eks:*` on `*`. The plan analyzer reports blocking risk signals for replacement, subnet coverage, capacity, ingress, IAM, and destroy actions. Split the replacement, networking, IAM, and capacity changes into separately reviewed plans with rollback notes.

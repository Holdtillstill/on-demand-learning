...
grep -q "Verified structural artifacts for 10 Platform Academy portfolio labs." "$REVIEW_PACK_DIR/portfolio-artifact-contract.txt" || fail "review pack portfolio artifact contract missing verifier output"
grep -q "Verified Platform Academy content counts: courses=21, lessons=84, labs=21, portfolio=10, resources=320, packs=22, questions=219." "$REVIEW_PACK_DIR/content-count-contract.txt" || fail "review pack content count contract missing verifier output"

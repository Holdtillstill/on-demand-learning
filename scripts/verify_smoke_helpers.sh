...
grep -q "Review changed paths:" <<<"$release_evidence_output" || fail "release evidence output did not include branch review scope"
...
grep -q "| Lab | Portfolio focus | Structural gate | Mode | Track | Time | Learner files | Source files | Evidence self-check | False-lead artifact |" <<<"$lab_matrix_output" || fail "lab matrix output did not include table header"
...
grep -q 'design-production-eks-review.*triage-notes.md' <<<"$lab_matrix_output" || fail "lab matrix missing triage-notes false-lead artifact"
grep -q 'trace-network-path.*hop-trace.md' <<<"$lab_matrix_output" || fail "lab matrix missing hop-trace false-lead artifact"
grep -q 'Rows with `Structural gate` set to `yes`' <<<"$lab_matrix_output" || fail "lab matrix missing structural gate note"
grep -q 'False-lead artifacts are either `triage-notes.md` or the network-specific `hop-trace.md`' <<<"$lab_matrix_output" || fail "lab matrix missing false-lead artifact note"
...
grep -q "Verified structural artifacts for 17 Platform Academy portfolio labs." "$REVIEW_PACK_DIR/portfolio-artifact-contract.txt" || fail "review pack portfolio artifact contract missing verifier output"
grep -q "Verified Platform Academy content counts: courses=21, lessons=84, labs=21, portfolio=17, resources=320, packs=22, questions=219." "$REVIEW_PACK_DIR/content-count-contract.txt" || fail "review pack content count contract missing verifier output"

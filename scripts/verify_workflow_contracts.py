...
def verify_platform_static_smoke_workflow() -> None:
    ...
    require("/api/events" in browser_smoke_script, f"{name} browser smoke must reject legacy event telemetry")
    require("on-demand-demos.bozhi.dev" in browser_smoke_script, f"{name} browser smoke must reject the removed visitor origin")
    require("no telemetry requests" in browser_smoke_script, f"{name} browser smoke must report no telemetry requests")
    summary_step = step_by_name(smoke_steps, "Write smoke summary", name)

...
- Nine practical labs are flagged as portfolio-grade in the catalog and UI; their key YAML, JSON, workflow, alert-rule, and Terraform plan artifacts are structurally verified by `make platform-lab-artifact-contract`.
...
- Lab review matrix: `make platform-lab-matrix` prints portfolio focus and structural-gate columns so reviewers can route the nine portfolio-grade labs separately from the broader full-lab set.

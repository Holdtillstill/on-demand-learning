...
Ten practical labs are flagged as portfolio-grade in the API and lab UI: `trace-service-to-pod`, `debug-crashloop-imagepull`, `review-yaml-before-apply`, `diagnose-eks-ip-exhaustion`, `trace-network-path`, `review-terraform-eks-plan`, `debug-irsa-access-denied`, `trace-argocd-drift`, `design-safe-release-pipeline`, and `write-slo-backed-runbook`. Their release gate parses key YAML, JSON, workflow, alert-rule, Terraform plan, and captured EKS evidence artifacts structurally, so the broken, risky, and fixed states are proven by contract instead of only by text search.

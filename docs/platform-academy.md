...
Nine practical labs are flagged as portfolio-grade in the API and lab UI: `trace-service-to-pod`, `debug-crashloop-imagepull`, `review-yaml-before-apply`, `trace-network-path`, `review-terraform-eks-plan`, `debug-irsa-access-denied`, `trace-argocd-drift`, `design-safe-release-pipeline`, and `write-slo-backed-runbook`. Their release gate parses key YAML, JSON, workflow, alert-rule, and Terraform plan artifacts structurally, so the broken, risky, and fixed states are proven by contract instead of only by text search.

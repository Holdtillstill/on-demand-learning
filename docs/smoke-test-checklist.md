...
- `make platform-browser-smoke WEB_BASE=https://...` passes against the Platform Academy web origin, including the 9 portfolio-grade lab UI signals and filter, all course, lesson, lab detail, resource detail, and interview-pack deep links, active-pack and visible-pack interview cram-sheet downloads, custom study-plan save/download, lab workbook save, lab evidence report download, guest profile export, and invalid backup import validation.

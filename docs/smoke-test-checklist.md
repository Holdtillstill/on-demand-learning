...
- `SMOKE_EXPECT_CLEAN_SPA_ROUTING=true WEB_BASE=https://platform-academy.bozhi.dev npm --prefix apps/platform-academy run smoke:static-host` passes after the CloudFront Function router is deployed; this verifies app deep links are not being served through CloudFront error fallback and that static API guardrails return JSON.
...
- `SMOKE_EXPECT_CLEAN_SPA_ROUTING=true` makes static-host smoke assert clean CloudFront SPA routing and JSON `404` API guardrails. Use it for the static deploy workflow and for manual verification after edge-router changes.
